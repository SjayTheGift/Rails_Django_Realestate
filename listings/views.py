...
from .serializers import ListingSerializer, listingDetailSerializer, ContactSerializer
...
class SearchView(generics.ListAPIView):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if "home_type" in self.request.GET:
            home_type = data['home_type']
            queryset = queryset.filter(home_type__iexact=home_type)

        if "bathrooms" in self.request.GET:
            bathrooms = data['bathrooms']
            print(bathrooms)
            queryset = queryset.filter(bathrooms__gte=bathrooms)

        if "sqft" in self.request.GET:
            sqft = data['sqft']
            queryset = queryset.filter(sqft__gte=sqft)
        

        # keywords = data['keywords']
        # queryset = queryset.filter(city__icontains=keywords)
